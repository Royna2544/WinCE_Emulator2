...
- Continue the real drawing/paint bridge. The host presenter now displays an exact `800x480` framebuffer, nested child windows restore their saved backing on destroy, and stale warning pixels no longer survive splash teardown. Richer common controls, complete host input-to-guest message translation, and remaining color-format edge cases still need work.
