...
- MSBuild succeeded after the host presenter and module-scoped `commctrl.dll` bridge changes.
- Smoke `v2_synth_system_smoke52_headless.log` with explicit SDK DLL directory arguments, `--registry C:\Users\royna\Downloads\INAVI\regs.json`, and `--headless` parsed resources, loaded the real SDK/user DLL set where present, mapped synthetic `commctrl.dll`, `WINSOCK.dll`, `ole32.dll`, and `OLEAUT32.dll`, created a host presenter HWND for guest HWND `0x00010007`, and reached the same honest idle stop: `GetMessageW blocking with empty guest queue`, `UC_ERR_OK pc=0x70002ae8 ra=0x500245c8`.
- Smoke52 has no remaining `unsupported by translate layer` calls before the idle stop. Current smoke does not reach any `commctrl.dll` calls, but the stale blanket dispatch path is gone.
- GUI smoke `v2_synth_system_gui52.log` verified default non-headless mode enters `entering host GUI message loop; close the presenter window to exit` after the guest destroys its main HWND following the language warning. The PowerShell harness closed the retained host presenter cleanly.
